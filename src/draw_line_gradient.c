...
/*   Updated: 2023/10/29 12:32:50 by sbalk            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int interpolateComponent(int start, int end, float t)
{
	return start + (int)((end - start) * t);
}

/* Interpolate color for every channel */
int interpolateColor(int startColor, int endColor, float t)
{
	t_rgb	start;
	t_rgb	end;
	t_rgb	interpolated;
	...
	interpolated.r = interpolateComponent(start.r, end.r, t);
	interpolated.g = interpolateComponent(start.g, end.g, t);
	interpolated.b = interpolateComponent(start.b, end.b, t);
...
int next_gradient_col(int s_col, int e_col, int step, int steps)
{
	...
	t = (float) step / steps;
	current_color = interpolateColor(s_col, e_col, t);
